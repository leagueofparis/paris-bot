...
function determineVoiceUpdate(oldState, newState) {
	...
	else return VoiceUpdateEnum.UNKNOWN;
}
...
async function onJoin(client, oldState, newState) {
	...
	if (oldState.channelID !== null) {
		...
		if (
			channel.parentId === client.ServerSettings.voiceCreatorCategoryID &&
			channel.id !== client.ServerSettings.voiceCreatorChannelID &&
			channel.name !== "mods"
		) {
	...
	if (newState.channelId === client.ServerSettings.voiceCreatorChannelID) {
		client.VoiceChannelManager.CreateVoiceChannel(
			guild,
			member,
			function (channelID) {
				try {
	...
	if (member.roles.cache.has(fun.id)) {
		...
		if (member.user.id === "105858401497546752") {
			//jangles
			client.VCM.PlaySound(
				newState.channelId,
				guild.id,
				guild.voiceAdapterCreator,
				"./assets/sounds/hello.mp4",
				0
			);
		} else if (member.user.id === "101755961496076288") {
			//erne
			client.VCM.PlaySound(
				newState.channelId,
				guild.id,
				guild.voiceAdapterCreator,
				"./assets/sounds/erne_sus.mp4",
				2500
			);
		} else if (member.user.id === "317753455190081536") {
			//robin
			client.VCM.PlaySound(
				newState.channelId,
				guild.id,
		...
		} else if (member.user.id === "231605256398569474") {
			//christine
			client.VCM.PlaySound(
				newState.channelId,
				guild.id,
				guild.voiceAdapterCreator,
				"./assets/sounds/do_not_come.mp4",
				2500
			);
		} else if (member.user.id === "132342534641811457") {
			//ace
			client.VCM.PlaySound(
				newState.channelId,
				guild.id,
				guild.voiceAdapterCreator,
				"./assets/sounds/ace_intro.mp4",
				2500
			);
		} else if (member.user.id === "100044596725170176") {
			//lorenz
			client.VCM.PlaySound(
				newState.channelId,
				guild.id,
				guild.voiceAdapterCreator,
				"./assets/sounds/lorenz_intro.mp4",
				2500
			);
		} else if (member.user.id === "477226464740245506") {
			//ring
			client.VCM.PlaySound(
				newState.channelId,
				guild.id,
				guild.voiceAdapterCreator,
				"./assets/sounds/ring_intro.mp4",
				2500
			);
		} else {
			client.VCM.PlaySound(
				...
				guild.id,
				guild.voiceAdapterCreator,
				"./assets/sounds/kick_your_ass.mp3",
				2500
			);
		}
	}
...
function onStartStream(client, oldState, newState) {
	...
	if (member.roles.cache.has(fun.id)) {
		client.VCM.PlaySound(
			...
			guild.id,
			guild.voiceAdapterCreator,
			"./assets/sounds/turn_that_off.mp3",
			2500
		);
	}
